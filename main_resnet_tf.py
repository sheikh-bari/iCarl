...
class_means    = np.zeros((128,nb_groups*nb_cl,2,nb_groups))
loss_batch     = []
files_protoset = []
labels_protoset = []
...
for itera in range(nb_groups):
  ...
  if itera == 0:
    ...
    # No distillation
    variables_graph,variables_graph2,scores,scores_stored = utils_icarl.prepare_networks(gpu,image_batch, nb_cl, nb_groups, keep_prob)
  ...
  with tf.Session(config=config) as sess:
    ...
    alexnet.save_model(save_path+'model-iteration'+str(nb_cl)+'-%i.pickle' % itera, scope='ResNet18', sess=sess)
